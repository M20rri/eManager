...
using eManager.Web.Startup;
using AngleSharp.Html.Dom;
using AngleSharp.Html.Parser;
using Microsoft.AspNetCore.Hosting;
using Shouldly;

namespace eManager.Web.Tests
{
    public abstract class eManagerWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static eManagerWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(eManagerWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<eManagerDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<eManagerDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<eManagerDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<eManagerDbContext, Task<T>> func)
        {
            T result;
